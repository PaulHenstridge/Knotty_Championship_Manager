score_reports = [
    ...
    "{team_name} have scored a stunning goal by {scorer}! - {score}",
    "A well worked goal from {team_name} courtesy of {scorer} - {score}",
    "{team_name} split the {other_team_name} defence and {scorer} scored a classic.  - {score}",
    "{team_name} on the attack! He headbutts a {other_team_name} defender... GOAL by {scorer}!!!  - {score}",
    "{team_name} attacking from wide, early ball to the striker {scorer}, GOAL!  - {score}",
    "{team_name} try a long range shot by {scorer}, and its in! What a goal!  - {score}",
    "{other_team_name} try to play out of defence, but take a stick to the face.... GOAL for {team_name} by {scorer}!!  - {score}",
    "Insane knotty skills from {team_name}, sublime violence by {scorer}... and its a GOAL!! - {score}",
    "{team_name}'s goalkeeper is advancing up the field! He finds {scorer} ... GOAL!!! - {score}",
    "{other_team_name}'s keeper is caught in no man's land!  A high lob out of defence by {scorer}... Its a GOAL!!! - {score}",

    # ... [other reports follow the same pattern, including {scorer}] ...

    "{other_team_name} press ahead, but a brutal smash to the kidneys drops their player to the floor. {team_name} pick up the ball and move rapidly down the right wing. It goes to {scorer} ... GOAL!!! - {score}",
    "{other_team_name}'s goalkeeper ventures far out, only to trip over his own feet! {team_name} seizes the moment, a quick one-two and {scorer} slots it home! The crowd erupts as the net bulges - GOAL!!! - {score}",
    "{other_team_name} tries a risky pass in midfield, but it's intercepted! {team_name} charges forward, a dazzling display of footwork and finesse by {scorer}. A thunderous strike and it's in the back of the net - GOAL!!! - {score}",
    "{other_team_name}'s defense is in disarray, struggling with a flurry of sticks and elbows. {team_name} exploits the chaos, a sharp pass to {scorer} who doesn't miss. A blistering goal that leaves the keeper stunned - GOAL!!! - {score}",
    "The ball is loose after a fierce tackle on an {other_team_name} midfielder. {team_name} pounces, a rapid sprint, a cross, and a spectacular header by {scorer}! The fans are on their feet - GOAL!!! - {score}",
    "In a daring move, {other_team_name}'s defender attempts a backwards somersault but fluffs it. {team_name}'s winger snatches it, darts past two defenders, and with a swish by {scorer}, it's a beautiful goal - GOAL!!! - {score}",
    "{other_team_name}'s forward fumbles the ball at a crucial moment. {team_name} capitalizes, breaking away with speed. A precise shot from the edge of the box by {scorer}, and it's a goal! - GOAL!!! - {score}",
    "{other_team_name} looks strong on the attack, but their striker's showboating costs them. {team_name} intercepts, a swift counter-attack, a strike from distance by {scorer}, and it's a screamer! - GOAL!!! - {score}",
    "After a failed corner kick, {other_team_name} leaves their goal vulnerable. {team_name} takes advantage, a long pass, a sprint, and a sublime chip over the keeper by {scorer} - GOAL!!! - {score}",
    "{other_team_name} is caught napping as their defender misjudges a header. {team_name} is quick to react, a lob to the forward who volleys it in mid-air by {scorer}. The net shakes - it's a magnificent goal! - GOAL!!! - {score}",
    "{other_team_name}'s goalie parries a shot but sends it straight to a {team_name} player. A quick adjustment, a powerful kick by {scorer}, and the ball finds its way home - GOAL!!! - {score}"
]
